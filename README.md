...
|---|---|---|---|---|
| Verra registry | `oracle/verra-adapter.ts` | `VERRA_REGISTRY_URL` (Verra-compatible VCS registry) | `VERRA_VCS` | `VerraProjectSchema`, `VerraMonitoringReportSchema` |
| Satellite | `oracle/satellite-processor.ts` | `SATELLITE_API_URL` (Sentinel-2 / Landsat NDVI stats) | `REMOTE_SENSING` | `SatelliteSceneSchema`, `SatelliteProjectConfigSchema` |
| IoT | `oracle/iot-aggregator.ts` | `IOT_API_URL` (in-situ soil sensors) | `IOT_SENSORS` | `IoTSensorReadingSchema`, `IotProjectConfigSchema` |
| Blue carbon | `oracle/blue-carbon-adapter.ts` | `BLUE_CARBON_API_URL` (mangrove/seagrass/saltmarsh plot surveys) | `BLUE_CARBON` | `BlueCarbonSurveySchema`, `BlueCarbonProjectConfigSchema` |
...
| **Phase 7** | Blue carbon bond support | 🟢 Delivered — `CreditType::BlueCarbon`, `BLUE_CARBON` oracle adapter, methodology docs |
